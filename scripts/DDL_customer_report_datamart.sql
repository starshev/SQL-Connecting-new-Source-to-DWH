...
CREATE TABLE IF NOT EXISTS dwh.customer_report_datamart (
    ...
    platform_money BIGINT NOT NULL, -- сумма, которую заработала платформа от заказчика за месяц (10% от суммы его покупок)
    ...
    top_product_type VARCHAR NOT NULL, -- самая популярная категория товаров у этого заказчика за месяц
    ...
    CONSTRAINT customer_report_datamart_fk FOREIGN KEY (top_craftsman_id) REFERENCES dwh.d_craftsman (craftsman_id)
);